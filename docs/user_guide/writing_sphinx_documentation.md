...
Let us say that `qsat_tender_scraper/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a ReST file, and add the following line to reference the `qsat_tender_scraper` module:

```rest
.. currentmodule:: qsat_tender_scraper
```
...
Let us say that `qsat_tender_scraper/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a Markdown file, and add the following line to reference the `qsat_tender_scraper` module:
